...
<body style='width: 100%; overflow: hidden;'>
    ...
    <input type="hidden" id='convo_id' value='<?=$convo_id?>'>
    <input type="hidden" id='user' value='<?php echo htmlspecialchars(json_encode($_SESSION['user'])); ?>'>
        ...
        <div class="col m-0 p-0"  style='height:100dvh; overflow:auto'>
            <div class="container-fluid h-100 d-flex flex-column p-0 m-0">
                ...
                <div class="row m-0">
                    <div class="col p-0">
                        <div class="header d-flex align-items-center justify-content-between bg-primary">
...
<script type='module'>
        ...
        $(document).ready(()=>{
            ...
            $.ajax({
                url : 'backend/fetch_convo_info.php',
                method : "post",
                data : { convo_id :convo_id },
                success : (response) => {
                    let res = JSON.parse(response);
                    $('#receiver_image').attr('src', `img/${res.profile_picture}`);
                    $('#receiver_name').text(res.full_name);
                }
            })
        ...
        let messages = collection(db, 'quicktiraTest');
        ...
        onSnapshot(q, (snapshot) => {
            ...
            Allmessage.forEach((message) => {
                if(message.sender == user.full_name){
